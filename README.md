...
`src/mcp.cyr` exposes the dataset as five read-only Model Context Protocol tools
over [bote](https://github.com/MacCracken/bote) — `itihas_era`,
`itihas_civilization`, `itihas_event`, `itihas_figure`, `itihas_timeline` — plus
the daimon host-registry integration.

It is **opt-in and not in the default bundle**: `dist/itihas.cyr` carries no bote
dependency, so consumers that only want the data pay nothing for it. To use the
tools, include the module and declare bote yourself:

```cyrius
include "itihas/dist/itihas.cyr"
include "itihas/src/mcp.cyr"
```

```sh
# itihas's own MCP build and tests activate the feature explicitly
